#!/usr/bin/env python3
"""Archive Meta quarterly earnings and filing documents locally."""
...
TICKER = "META"
CIK = "0001326801"
SEC_CIK_PATH = "1326801"
META_BASE_URL = "https://investor.atmeta.com"
META_FINANCIALS_URL = "https://investor.atmeta.com/financials/"
...
META_DOCUMENTS = {
    "Earnings Release": "earnings_release_pdf",
    "Earnings Slides": "earnings_slides_pdf",
    "Balance Sheet": "balance_sheet_xlsx",
    "Income Statement": "income_statement_xlsx",
}
...
def load_meta_targets(panel_path: Path) -> dict[str, dict[str, str]]:
...
def meta_financial_report_url() -> str:
    ...
    return f"{META_BASE_URL}/feed/FinancialReport.svc/GetFinancialReportList?{urllib.parse.urlencode(params)}"
...
def resolve_meta_url(path_or_url: str) -> str:
    return urllib.parse.urljoin(META_BASE_URL, path_or_url)


def collect_meta_feed_documents(
    targets: dict[str, dict[str, str]],
    source_dir: Path,
) -> list[dict[str, str]]:
    response = fetch_url_json(meta_financial_report_url())
    write_json(source_dir / "meta_q4_financial_reports.json", response)
    ...
    for fiscal_period, target in sorted(targets.items()):
        ...
        if not report:
            for title, document_kind in META_DOCUMENTS.items():
                documents.append(missing_document(target, document_kind, "", f"no Meta report row found for {title}"))
            documents.append(missing_document(target, "sec_filing_pdf", expected_form_type(fiscal_period), "no Meta report row found for SEC filing PDF"))
            continue

        docs_by_title = {
            str(doc.get("DocumentTitle") or ""): doc
        ...
        for title, document_kind in META_DOCUMENTS.items():
            ...
            if not doc:
                documents.append(missing_document(target, document_kind, "", f"no {title} document found in Meta IR feed"))
                continue
            documents.append(
                {
                    **target,
                    ...
                    "form_type": "",
                    "title": title,
                    "official_page_url": META_FINANCIALS_URL,
                    "source_url": resolve_meta_url(doc["DocumentPath"]),
                    "note": "",
                    "accession_number": "",
                    "filing_date": "",
                }
        ...
        form_title = expected_form_type(fiscal_period)
        form_doc = docs_by_title.get(f"Form {form_title}")
        if not form_doc:
            documents.append(missing_document(target, "sec_filing_pdf", form_title, f"no Form {form_title} row found in Meta IR feed"))
        else:
            form_url = resolve_meta_url(form_doc["DocumentPath"])
            if urllib.parse.urlparse(form_url).path.lower().endswith(".pdf"):
                documents.append(
                    {
                        **target,
                        "document_kind": "sec_filing_pdf",
                        "form_type": form_title,
                        "title": f"Form {form_title}",
                        "official_page_url": META_FINANCIALS_URL,
                        "source_url": form_url,
                        "note": "",
                        "accession_number": "",
                ...
                documents.append(
                    missing_document(
                        ...
                        "sec_filing_pdf",
                        form_title,
                        f"Meta IR feed points to filing detail page instead of direct PDF: {form_url}",
                    )
                )
...
def collect_documents(targets: dict[str, dict[str, str]], source_dir: Path) -> list[dict[str, str]]:
    documents = collect_meta_feed_documents(targets, source_dir)
    time.sleep(0.2)
...
def write_readme(output_dir: Path, manifest_rows: list[dict[str, Any]]) -> None:
    ...
    text = f"""# Meta IR Document Archive
...
- Meta financials: {META_FINANCIALS_URL}
- SEC submissions JSON: {SEC_SUBMISSIONS_URL}
"""
    (output_dir / "README_META.md").write_text(text, encoding="utf-8")
...
def main(argv: list[str] | None = None) -> int:
    ...
    targets = load_meta_targets(args.panel_path)
    if not targets:
    ...
    write_json(args.output_dir / "manifest_META.json", manifest_rows)
    write_csv(args.output_dir / "manifest_META.csv", manifest_rows)
    write_readme(args.output_dir, manifest_rows)

    downloaded = sum(1 for row in manifest_rows if row.get("status") == "downloaded")
    missing = sum(1 for row in manifest_rows if row.get("status") == "missing")
    errored = sum(1 for row in manifest_rows if row.get("status") == "error")
    print(f"Wrote Meta IR document archive manifest to {args.output_dir}")
