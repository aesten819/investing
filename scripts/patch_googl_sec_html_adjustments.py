#!/usr/bin/env python3
"""Patch GOOGL derived financial data from archived GOOG SEC inline XBRL HTML."""
...
def load_sec_html_by_quarter(sec_html_dir: Path) -> dict[str, dict[str, str]]:
    ...
    for path in sorted(sec_html_dir.glob("goog-*.html")):
        ...
        results[quarter_label(report_period)] = {
...
def patch_googl_debt_from_sec_html(
...
def patch_googl_capex_from_sec_html(
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not html_by_quarter:
        print(f"No downloaded GOOG SEC HTML filings found in {args.sec_html_dir}", file=sys.stderr)
        return 1
