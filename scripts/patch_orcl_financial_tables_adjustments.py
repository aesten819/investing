#!/usr/bin/env python3
"""Patch ORCL derived financial data from archived Oracle financial table XLSX files."""
...
from validate_orcl_financials_against_xlsx import (
    BALANCE_METRICS,
    CASH_FLOW_METRICS,
    INCOME_METRICS,
    TICKER,
...
DEBT_DEFINITION = (
    "pure_financial_debt: notes payable and other borrowings, current + notes payable and other "
    "borrowings, non-current; excludes operating lease liabilities"
)
...
def annotate_orcl_debt(
    ...
    for row in balance_rows:
        ...
        if xlsx_info is None:
            continue
        row["debt_source"] = "orcl_financial_tables_xlsx:pure_financial_debt"
...
def annotate_orcl_capex(
    ...
    for row in cash_rows:
        ...
        row["capex_source"] = "orcl_financial_tables_xlsx:Capital expenditures"
        row["capex_sec_accession_number"] = ""
        row["capex_sec_filed"] = ""
        row["capex_sec_frame"] = "quarterly Oracle financial tables XLSX"
...
def patchable_fields(metric_defs: list[Any]) -> list[str]:
    return list(dict.fromkeys(metric.metric for metric in metric_defs if metric.comparable))


def write_orcl_source_files(
    data_dir: Path,
    balance_rows: list[dict[str, Any]],
    cash_rows: list[dict[str, Any]],
...
def main(argv: list[str] | None = None) -> int:
    ...
    debt_annotated = annotate_orcl_debt(balance_rows, xlsx_by_fiscal_period)
    capex_annotated = annotate_orcl_capex(cash_rows, xlsx_by_fiscal_period)

    write_orcl_source_files(args.data_dir, balance_rows, cash_rows, income_rows)
    ...
    print(f"Patched ORCL balance rows from Oracle financial tables XLSX: {balance_touched} rows, {balance_changes} value changes; debt annotations {debt_annotated}")
    print(f"Patched ORCL cash flow rows from Oracle financial tables XLSX: {cash_touched} rows, {cash_changes} value changes; capex annotations {capex_annotated}")
    print(f"Patched ORCL income rows from Oracle financial tables XLSX: {income_touched} rows, {income_changes} value changes")
