#!/usr/bin/env python3
"""Translate memory-semiconductor news cards to Korean with DeepSeek."""
...
DEFAULT_ARTICLES_PATH = Path("data/memory-news/articles.json")
DEFAULT_CACHE_PATH = Path("data/memory-news/translation_cache.json")
...
def build_translation_messages(article: dict[str, Any]) -> list[dict[str, str]]:
    ...
    system_prompt = (
        "You translate memory semiconductor news cards for a Korean investment research page. "
        "Return JSON only. Keep company names, product names, ticker-like terms, and these terms in English "
        f"when natural: {', '.join(GLOSSARY_TERMS)}. "
